...
def main():
    ...
    current_vm = get_obj(content, [vim.VirtualMachine], ansiblemodule.params['vmware_dest_vm'])
    ...
    if current_vm:
        ...
        result = wait_for_task(task)
        if result['task_done']:
            ansiblemodule.exit_json(changed=True, msg="The current VM has been deleted")
        else:
            ansiblemodule.fail_json(msg="Something failed deleting the VM")

    else:
        ansiblemodule.exit_json(changed=False, msg="Destination VM not found, skipping...")
